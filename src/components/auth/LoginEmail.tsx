...
class LoginEmail extends React.Component<ILoginEmailProps, any> {
      ...
      <Form>
        ...
        <div>
          ...
          {!this.props.isAuthenticating && (
            <Button variant="contained" type="submit" disabled={isSubmitting} color="primary" className="buttom-sing-spotify">
              LOG IN
             </Button>
          )}
        </div>
  ...
  private renderPasswordInput = ({
    ...
    return (
      <div className="login-input-container">
          <Input id="password-input" className="login-email" type={this.state.showPassword ? 'text':'password'} placeholder='Password' error={passwordError} {...field} />
        <Icon className="login-eye-icon" onClick={this.showPassword}> remove_red_eye</Icon>
      </div>
    )
  }
